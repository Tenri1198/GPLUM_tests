...
inline void Collision0::inputPair(Tpsys & pp,
    ...
    flag_imp = pp[id_c_imp].flag_gd;    //
    flag_tar = pp[id_c_tar].flag_gd;    //
    ...
    std::cerr << std::fixed << std::setprecision(8)
              ...
              << vel_imp.x << "\t" << vel_imp.y << "\t" << vel_imp.z <<"\t"<<pp[id_c_imp].acc_gd.x<<"\t"<<pp[id_c_imp].acc_gd.y<<"\t"<<pp[id_c_imp].acc_gd.z<<"\t"<<pp[id_c_imp].flag_gd<<"\t"<<pp[id_c_imp].mass<<std::endl
              << "Tar_pos_vel:     " << pos_tar.x << "\t" << pos_tar.y << "\t" << pos_tar.z << "\t"
              << vel_tar.x << "\t" << vel_tar.y << "\t" << vel_tar.z <<"\t"<<pp[id_c_tar].acc_gd.x<<"\t"<<pp[id_c_tar].acc_gd.y<<"\t"<<pp[id_c_tar].acc_gd.z<<"\t"<<pp[id_c_tar].flag_gd<<"\t"<<pp[id_c_tar].mass<<std::endl;
}
                                    ...
                                    PS::S64 & id_next)
{
    ...
    flag_imp = pp[id_c_imp].flag_gd;    //
    flag_tar = pp[id_c_tar].flag_gd;    //
    PS::F64 mass_temp_imp = 0.;
    PS::F64 mass_temp_tar = 0.;
    ///////////////////
    /*   Accretion   */
    ///////////////////
    //Mass & Radius
    ...
    if ( !flag_merge ) {
        ...
        if ( r_planet_tar_new < 0. ) {
            if(flag_tar==0 && flag_imp==1)
            {
                ...
                mass_temp_imp = mass_imp*1.844028699792144e-09/5.028e-20;
                mass_rem = mass_temp_imp + mass_tar - mass_frag;
                pp[id_c_tar].r_planet*= pow(mass_rem/mass_tar, 1./3.);
                std::cerr<< std::scientific << std::setprecision(16)<< "New mass before mass addition [tar:imp=0:1]: " << pp[id_c_tar].mass<<" : " <<mass_temp_imp<<std::endl;
                std::cerr<< std::scientific << std::setprecision(16)<<"New targetter size[tar:imp=0:1]: "<<pp[id_c_tar].r_planet<<std::endl;
            }
            else if(flag_tar==1 && flag_imp==0)
            {
                ...
                mass_temp_tar = mass_tar*1.844028699792144e-09/5.028e-20;
                mass_rem = mass_imp + mass_temp_tar - mass_frag;
                pp[id_c_tar].r_planet*= pow(mass_rem/mass_tar, 1./3.);
                std::cerr<< std::scientific << std::setprecision(16)<< "New mass before mass addition [tar:imp=1:0]: " << mass_temp_tar<<" : " <<pp[id_c_imp].mass<<std::endl;
                std::cerr<< std::scientific << std::setprecision(16)<<"New targetter size[tar:imp=1:0]: "<<pp[id_c_tar].r_planet<<std::endl;
                pp[id_c_tar].f = 1.0;
            }
            else
            {
                std::cerr<< std::scientific << std::setprecision(16)<< "Old targetter size[tar:imp=1:1 or 0:0]: " << pp[id_c_tar].r_planet<<"\tTarget ID:Impactor ID " << id_tar<<":"<<id_imp<<std::endl;//
                std::cerr<< std::scientific << std::setprecision(16)<< "Old mass [tar:imp=1:1 or 0:0]: " << pp[id_c_tar].mass<<" : " <<pp[id_c_imp].mass<<std::endl;
                pp[id_c_tar].r_planet *= pow(mass_rem/mass_tar, 1./3.);
                std::cerr<< std::scientific << std::setprecision(16)<< "New mass before mass addition [tar:imp=1:1 or 0:0]: " << pp[id_c_tar].mass<<" : " <<pp[id_c_imp].mass<<std::endl;
                std::cerr<< std::scientific << std::setprecision(16)<<"New targetter size[tar:imp=1:1 or 0:0]: "<<pp[id_c_tar].r_planet<<std::endl;
            }//pp[id_c_tar].r_planet *= pow(mass_rem/mass_tar, 1./3.);
            //pp[id_c_tar].setRPlanet(mass_rem);
        } else {
            pp[id_c_tar].r_planet *= pow((mass_imp - mass_frag)/mass_imp, 1./3.);
        }
    }
    ...
    //if ( !HitAndRun ) {
    if ( flag_merge ) {
        std::cout<<"flag_gd checker[impactor:target] collision.h before"<<pp[id_c_imp].id<<":"<<pp[id_c_imp].flag_gd<<" "<<pp[id_c_tar].id<<":"<<pp[id_c_tar].flag_gd<<std::endl;
   ...
   // std::cout<<"flag_gd checker[impactor:target] before"<<pp[id_c_imp].id<<":"<<pp[id_c_imp].flag_gd<<" "<<pp[id_c_tar].id<<":"<<pp[id_c_tar].flag_gd<<std::endl;
    //pp[id_c_imp].flag_gd &= pp[id_c_tar].flag_gd;
    //pp[id_c_tar].flag_gd = pp[id_c_imp].flag_gd;
    for (iterator it = imp_range.first; it != imp_range.second; ++it){
        PS::S32 id_i = it->second;
        pp[id_i].pos = pos_imp_new;
        pp[id_i].vel = vel_imp_new;
       // pp[id_i].flag_gd &= pp[id_c_tar].flag_gd;
       // pp[id_c_tar].flag_gd = pp[id_c_imp].flag_gd;
    }    
    ...
    std::cout<<"flag_gd checker[impactor:target] collisionA.h after"<<pp[id_c_imp].id<<":"<<pp[id_c_imp].flag_gd<<" "<<pp[id_c_tar].id<<":"<<pp[id_c_tar].flag_gd<<std::endl;
    //if ( !HitAndRun ) {
    if ( flag_merge ) {
    ...
    std::cerr << std::scientific << std::setprecision(15)
              ...
              << vel_imp_new.x << "\t" << vel_imp_new.y << "\t" << vel_imp_new.z <<"\t"<<pp[id_c_imp].flag_gd<<"\t"<<pp[id_c_imp].mass<<std::endl
              << "Tar_pos_vel_new: " << pos_tar_new.x << "\t" << pos_tar_new.y << "\t" << pos_tar_new.z << "\t"
              << vel_tar_new.x << "\t" << vel_tar_new.y << "\t" << vel_tar_new.z <<"\t"<<pp[id_c_tar].flag_gd<<"\t"<<pp[id_c_tar].mass<<std::endl;
